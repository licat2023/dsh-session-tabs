...
`
    ...
    function SessionTabs(props) {
      ...
      // the home state (no session selected); already home does nothing.
      const goBack = () => {
        if (navIndex <= 0) {
          if (navIndex === -1) return
          navIndex = -1
          // A deliberate back-to-home is not a terminal state: forward should
          // still restore the first history entry.
      ...
      React.useEffect(() => {
        ...
        // The selection already sits at the current history position: this
        // render is just the same session (nothing to record). Note the
        // position check, NOT indexOf: from home (navIndex -1) a brand-new
        // session must still be appended.
        if (navStack[navIndex] === current) return
        // Fresh navigation (or the first selection): append and truncate.
        navStack = navStack.slice(0, navIndex + 1).concat([current])
      ...
      const closeTab = (id) => {
        ...
        setOrder(next)
        if (id === currentRef.current && sessions !== undefined) {
          if (next.length > 0) {
          ...
          } else if (typeof sessions.clear === 'function') {
            // The strip is empty: drop the selection into the home state
            // (New Session hero) so the layout stays consistent. The home
            // state is now a terminal endpoint — forward has nothing to
            // restore to — while back still recovers prior sessions.
